...
public class CSVLineRecordReader extends RecordReader<LongWritable, List<Text>> {
	...
	public void init(InputStream is, Configuration conf) throws IOException {
		//System.out.println("CSV Record reader init() method");
	...
	protected int readLine(List<Text> values) throws IOException {
		//System.out.println("CSV Record reader readLine() called");
		...
		while ((i = in.read()) != -1) {
			...
			numRead++;
			sb.append(c);
			//System.out.println(String.format("char - %s, sb - %s", c, sb.toString()));
		...
		//System.out.println(String.format("value lenght - %d, 6th arg - %s, 5th arg - %s", values.size(), values.get(6), values.get(5)));
		//System.out.println(String.format("Returning read bytes = %d", numRead));
		return numRead;
	}
	...
	public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException {
		//System.out.println("INITIALIZE CSV Record reader");
		...
		System.out.println(split.getPath()); //REMOVE THIS
